//! Reimplements core logic and types from `std::io` in an `alloc`-friendly fashion.
...
// I/O errors can never occur in no-std mode. All our no-std I/O implementations are infallible.
pub struct Error;
